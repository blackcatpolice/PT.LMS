...
namespace Pagetechs.Framework.Qinius
{
    ...
    { 
        ...
        string bucket;
        ZoneID zoneID;
        public QiniuTool(string _AK, string _SK, string _bucket, string _zoneID)
        {
            AK = _AK;
            SK = _SK;
            bucket = _bucket;
            zoneID = (ZoneID)Enum.Parse(typeof(ZoneID), _zoneID);
        }
        ...
        public string GenerateToken(string fileName, string _bucket, ZoneID zoneID)
        {
        ...
        public async Task DeleteImage(string[] saveKey)
        {
            GenerateToken();
        ...
        public async Task UploadImage(string fileName, string saveKey)
        {
        ...
        public async Task UploadImage(Stream stream, string saveKey)
        {
            var token = GenerateToken();
            UploadManager um = new UploadManager(putThreshold: 4 * 1024 * 1024);
            um.SetChunkUnit(ChunkUnit.U4096K);
        ...
        public async Task UploadImage(byte[] bytes, string saveKey)
        {
            var token = GenerateToken();
            UploadManager um = new UploadManager(putThreshold: 4 * 1024 * 1024);
            um.SetChunkUnit(ChunkUnit.U4096K);
        ...
        public async Task<List<string>> ListFile(string prefix = "image")
        {
            GenerateToken();
            var mgr = new Qiniu.RS.BucketManager(macKey);
